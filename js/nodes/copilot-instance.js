...
export class CopilotInstance {
  ...
  #wireResize() {
    ...
    const onMove = (e) => {
      ...
      if (dir === 't') { const rawH = sh - dy; newH = Math.max(MIN_H, Math.min(MAX_H, rawH)); const maxDy = sh - newH; newT = st + Math.min(Math.max(dy, -10000), maxDy); }
